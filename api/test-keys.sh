...
SPIN_LOG=${SPIN_LOG:-"$SPIN_STATE_DIR/logs/spin-keys.log"}
...
step "get keys metadata"
read -r code out < <(request GET "/admin/realms/$REALM/keys")
expect_status "$code" "$out" 200
rm -f "$out"

step "get keys for missing realm"
MISSING_REALM="missing-$(date +%s)"
read -r code out < <(request GET "/admin/realms/$MISSING_REALM/keys")
