#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
BASE_URL=${BASE_URL:-http://127.0.0.1:3000}
REALM=${REALM:-demo}
SPIN_UP=${SPIN_UP:-0}
SPIN_BUILD=${SPIN_BUILD:-0}
SPIN_STATE_DIR=${SPIN_STATE_DIR:-"$ROOT_DIR/.spin"}
SPIN_MANIFEST=${SPIN_MANIFEST:-"$ROOT_DIR/spin.toml"}
SPIN_LOG=${SPIN_LOG:-"$SPIN_STATE_DIR/logs/spin-localization.log"}

request() {
  local method=$1
  local path=$2
  local data=${3-}
  local out
  out=$(mktemp)
  local code
  if [[ -n "$data" ]]; then
    code=$(curl -sS -o "$out" -w "%{http_code}" -X "$method" \
      -H "content-type: application/json" \
      --data "$data" \
      "$BASE_URL$path")
  else
    code=$(curl -sS -o "$out" -w "%{http_code}" -X "$method" "$BASE_URL$path")
  fi
  echo "$code" "$out"
}

request_text() {
  local method=$1
  local path=$2
  local text=$3
  local out
  out=$(mktemp)
  local code
  code=$(curl -sS -o "$out" -w "%{http_code}" -X "$method" \
    -H "content-type: text/plain" \
    --data "$text" \
    "$BASE_URL$path")
  echo "$code" "$out"
}

expect_status() {
  local code=$1
  local out=$2
  shift 2
  local ok=0
  for expected in "$@"; do
    if [[ "$code" == "$expected" ]]; then
      ok=1
      break
    fi
  done
  if [[ "$ok" != "1" ]]; then
    cat "$out" >&2
    echo "unexpected status: $code (expected: $*)" >&2
    exit 1
  fi
}

step() {
  echo "==> $1"
}

if [[ "$SPIN_UP" == "1" ]]; then
  if [[ "$SPIN_BUILD" == "1" ]]; then
    (cd "$ROOT_DIR/api" && cargo build --target wasm32-wasip2 --release)
    if [[ -d "$ROOT_DIR/db/init" ]]; then
      (cd "$ROOT_DIR/db/init" && cargo build --target wasm32-wasip2 --release)
    fi
  fi
  mkdir -p "$(dirname "$SPIN_LOG")"
  spin up -f "$SPIN_MANIFEST" --state-dir "$SPIN_STATE_DIR" --listen 127.0.0.1:3000 >"$SPIN_LOG" 2>&1 &
  SPIN_PID=$!

  cleanup() {
    if kill -0 "$SPIN_PID" 2>/dev/null; then
      kill "$SPIN_PID" 2>/dev/null || true
      wait "$SPIN_PID" 2>/dev/null || true
    fi
  }
  trap cleanup EXIT

  ready=0
  for _ in $(seq 1 50); do
    status=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/admin/realms" || true)
    if [[ "$status" != "000" ]]; then
      ready=1
      break
    fi
    sleep 0.2
  done

  if [[ "$ready" != "1" ]]; then
    echo "spin did not become ready; see $SPIN_LOG" >&2
    exit 1
  fi
fi

LOCALE=${LOCALE:-en}
KEY=${KEY:-app.name}
VALUE=${VALUE:-Keycloak Spin}

step "ensure realm exists"
read -r code out < <(request POST /admin/realms '{"realm":"'$REALM'","enabled":true}')
expect_status "$code" "$out" 201 409
rm -f "$out"

step "list localizations"
read -r code out < <(request GET "/admin/realms/$REALM/localization")
expect_status "$code" "$out" 200
rm -f "$out"

step "import localization locale"
read -r code out < <(request POST "/admin/realms/$REALM/localization/$LOCALE" '{"hello":"world"}')
expect_status "$code" "$out" 204
rm -f "$out"

step "get localization locale"
read -r code out < <(request GET "/admin/realms/$REALM/localization/$LOCALE")
expect_status "$code" "$out" 200
rm -f "$out"

step "put localization key"
read -r code out < <(request_text PUT "/admin/realms/$REALM/localization/$LOCALE/$KEY" "$VALUE")
expect_status "$code" "$out" 204
rm -f "$out"

step "get localization key"
read -r code out < <(request GET "/admin/realms/$REALM/localization/$LOCALE/$KEY")
expect_status "$code" "$out" 200
rm -f "$out"

step "delete localization key"
read -r code out < <(request DELETE "/admin/realms/$REALM/localization/$LOCALE/$KEY")
expect_status "$code" "$out" 204
rm -f "$out"

step "delete localization locale"
read -r code out < <(request DELETE "/admin/realms/$REALM/localization/$LOCALE")
expect_status "$code" "$out" 204
rm -f "$out"

step "get missing locale"
read -r code out < <(request GET "/admin/realms/$REALM/localization/$LOCALE")
expect_status "$code" "$out" 404
rm -f "$out"

echo "OK"
